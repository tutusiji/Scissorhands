...
const splitAndSaveImages = () => {
  ...
  bottomImageY = lineY;
  redrawImages(topImage, bottomImage);
};

// 重绘图像
const redrawImages = (topImage: ImageData, bottomImage: ImageData) => {
  if (ctx) {
    ctx.clearRect(0, 0, 500, 500);
    ctx.putImageData(topImage, 0, topImageY);
    ctx.putImageData(bottomImage, 0, bottomImageY);
  }
};
...
const handleMouseMove = (event: MouseEvent) => {
  if (isDragging && ctx) {
    ...
    const topImage = ctx.getImageData(0, 0, 500, lineY);
    const bottomImage = ctx.getImageData(0, lineY, 500, 500 - lineY);
    redrawImages(topImage, bottomImage);
  }
};
